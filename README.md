...
- `notebooks/02_phase_codec.ipynb` — `SpectrumCodec` footprint, synthetic forward, loss bars, encode/decode path.
- `notebooks/03_phase_codec_eval.ipynb` — pulls **`codec_v2`** / **`codec_v3`** checkpoints from W&B (needs `WANDB_API_KEY`) or from `checkpoints/<run_name>/best.pt`, plots **W&B `train/recon` curves** when `wandb_id` is set in `TRAINING_REGISTRY.yaml`, and overlays **flux reconstructions** on the same batch.
